...
def aws_crud():
    ...
    df = utility.bucket(json_data,bucket_name,file_path_name,"aws")
    lst1=list(df.columns)
    lst2 = static_list
    json_dict = {col: lst2 for col in lst1}
    json_data = json.dumps(json_dict)
...
def file_GE():
    ...
    print("\n")
    print("\n")
    exp1 = dict(dict_data['Expectation_Values']["UniqueExpectationValues"])
    exp2 = dict(dict_data["Expectation_Values"]["CommonExpectation"])
...
def mailer():
    ...
    print(data,"********")
    email_notification.send_email(data["sender_name"], data["receiver_mail"], result, data["Subject"])
    print("Success")
    return {"success":True}
